...
2. **Fetch unlabeled bugs** from JIRA:
   ...
   **MCP Mode (default - when --api is NOT specified)**:
   ...
   - Use this JQL query:
   ```
   project = OCPBUGS AND component in ("Networking / openshift-sdn", "Networking / ovn-kubernetes", "Networking / cloud-network-config-controller", "Networking / ingress-node-firewall", "Networking / cluster-network-operator", "Networking / network-tools") AND resolution = Unresolved AND (assignee in("core-networking-bot@redhat.com","bbennett@redhat.com") OR assignee is EMPTY) ORDER BY Rank DESC
   ```
   - Request fields: `summary,description,labels,components,issuelinks`
   - **IMPORTANT - Pagination**: The search may return paginated results with a `next_page_token`. You MUST continue fetching pages until no more pages are available (no `next_page_token` returned). Process ALL bugs across ALL pages.
   - **Label Filtering**: The JQL query does NOT pre-filter by labels. After fetching bugs, you must filter out bugs that already have an area label (any label starting with "SDN:" or "SDN-") yourself.
